import "./CleverLogo.css";

interface Props {
  className?: string;
  /** Any CSS colour; Clever's own default for the blue bar is white. */
  color?: string;
  title?: string;
}
 ...
 * The Clever wordmark.
 *
 * Outline and viewBox are taken verbatim from Clever's own `Logo` component
 * (src/Logo/index.tsx in clever-components) so the mark is the real one rather
 * than a redrawing. Like the original, the SVG is sized by `font-size`: the
 * image is 1em tall and keeps the 198:55 aspect ratio of its viewBox.
 */
export function CleverLogo({ className, color = "#fff", title }: Props) {
  return (
    <div className={["clever-logo", className].filter(Boolean).join(" ")}>
      <svg
        className="clever-logo__image"
        viewBox="0 0 198 55"
